...
class ProgramGuideManager<T> {
    ...
    companion object {
        ...
        private const val DAY_ENDS_NEXT_DAY_AT_HOUR = 11
        ...
        if (endUtcMillis > startUtcMillis) {
            for (channel in channels) {
                ...
                if (entries.isEmpty()) {
                    ...
                    val timelineEndsAt = timelineStartsAt.plusDays(7).withHour(DAY_ENDS_NEXT_DAY_AT_HOUR)

                    val timelineStartsAtMillis = System.currentTimeMillis()-(10*60*1000)
