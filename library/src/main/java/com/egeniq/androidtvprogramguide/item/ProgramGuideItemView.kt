...
class ProgramGuideItemView<T> : FrameLayout {
    ...
    fun setValues(
        ...
        title =
            if (title?.isEmpty() == true) "" else title
        ...
        timeView.setText(getFormattedTime(scheduleItem.startsAtMillis)+ " - "+getFormattedTime(scheduleItem.endsAtMillis))
        titleView.measure(
    ...
    private fun getFormattedTime(atMillis: Long): String {
        try {
            val date = Date(atMillis)
            val date_format = SimpleDateFormat("h:mm a")
